...
public class DomainWhiteListWorkFlowServiceImpl implements DomainWhiteListWorkFlowService {
    ...
    public Response createDomainWorkFlow(String rootOrg, String org, WfRequest wfRequest) {
        ...
        try {
            Optional<HashMap<String, Object>> optionalFieldValue = wfRequest.getUpdateFieldValues()
                    .stream()
                    .findFirst();

            if (optionalFieldValue.isEmpty()) {
                response.put(Constants.MESSAGE, "No update field values found in the workflow request.");
                response.put(Constants.STATUS, HttpStatus.BAD_REQUEST);
                return response;
            }
            HashMap<String, Object> updatedFieldValue = optionalFieldValue.get();
    ...
    private Response updateWfRequestAndDomainLookup(Response response, String rootOrg, String org, WfRequest wfRequest, String domainValue, int updatedCountValueForDomainRequest, WfDomainLookup wfDomainLookup) throws JsonProcessingException {
        ...
        if (HttpStatus.OK.compareTo((HttpStatus) wfResponse.getResult().get(Constants.STATUS)) == 0) {
            ...
            wfResponse.put(Constants.MSG_CODE, Constants.DOMAIN_REQUEST_CREATED);
        }
        return wfResponse;
    }
